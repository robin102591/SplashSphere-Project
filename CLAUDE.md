...
35. **Customer Display Routing**: `Transaction.PosStationId` (nullable) determines whether and where transaction lifecycle events broadcast to the customer-facing display. `IDisplayBroadcaster` (`SignalRDisplayBroadcaster` impl) sends `DisplayTransactionStarted/Updated/Completed/Cancelled` to `display:{branchId}:{stationId}` SignalR group on each event — driven by domain event handlers (`TransactionCreatedDisplayHandler`, `TransactionUpdatedDisplayHandler`, `TransactionCompletedDisplayHandler`, `TransactionCancelledDisplayHandler`). The broadcaster is a no-op when `PosStationId` is null, so admin-app transactions and any transactions created before this slice cost nothing. **Failures are swallowed and logged at warning** so a SignalR hiccup never breaks the completion pipeline. Payloads are deliberately customer-safe: `DisplayTransactionResultDto` / `DisplayCompletionResultDto` (Application layer, in `Features/Display/DTOs/`) contain only items, totals, payment, vehicle, customer name + tier, loyalty points, and footer messages — **never** employee names, commissions, costs, profit margins, or other internal data. The same DTO + `DisplayTransactionLoader` builds both the SignalR broadcast and the `GET /api/v1/display/current?branchId=…&stationId=…` reconnect-sync response, guaranteeing byte-identical payloads across both paths. The display device calls the reconnect-sync endpoint on initial connect and after every SignalR auto-reconnect.
36. **POS Station Selection**: The cashier picks a station from a navbar dropdown next to the branch picker (`StationSelector` in `pos-navbar.tsx`); the choice persists in `localStorage` under `pos-station-id`. Switching branch clears the station (its IDs may not exist in the new branch). `BranchProvider` exposes `stationId` / `stationName` / `stations` / `setStationId` alongside the existing branch state. `CreateTransactionCommand` accepts an optional `PosStationId` parameter (defaulted to null for backward compat); the POS submits whatever is currently selected. **Without a station selected, the customer display feature silently doesn't activate** — there is no hard gate; the cashier can still process transactions normally.
37. **Customer Display Plan Limits**: Two limits live on `PlanDefinition`: `MaxPosStationsPerBranch` (Starter 1, Growth 3, Enterprise ∞) and `MaxPromoMessages` (Starter 1, Growth 5, Enterprise 20). The station limit is enforced by `IPlanEnforcementService.CheckPosStationLimitAsync(tenantId, branchId)` and gated in `CreatePosStationCommandHandler` before insert — the count includes inactive stations so soft-deactivation can't bypass the cap. The promo cap is enforced in `UpdateDisplaySettingCommandHandler` (the FluentValidation `<=20` rule stays as a backstop ceiling). Per-branch display overrides remain feature-gated by `FeatureKeys.BranchDisplayOverrides` (Enterprise only). The admin UI reads `PlanLimitsDto` from `GET /billing/plan` to disable the "Add message" button at cap and surface a `{used}/{cap}` badge.
38. **Tenant Primary Brand Color**: `Tenant.PrimaryColorHex` (nullable, `#RRGGBB`) drives the customer display's "Brand" theme accent. Validated in `UpdateCompanyProfileCommandValidator` against `^#[0-9A-Fa-f]{6}$` and normalized to uppercase by the handler. Surfaces on `CompanyProfileDto` (admin form) and `DisplayBrandingDto` (display app). The display app sets a `--display-accent` CSS variable on the root element when theme is Brand, and the Brand theme's `accent` class resolves to `text-[color:var(--display-accent,#60A5FA)]`. Other themes ignore the variable entirely. Null = use the default blue.
